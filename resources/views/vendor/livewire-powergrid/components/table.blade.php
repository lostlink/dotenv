<x-livewire-powergrid::table-base :theme="$theme->table">
    <x-slot name="header">
        <tr class="{{ $theme->table->trClass }}"
            ...
            @if ($checkbox)
                <x-livewire-powergrid::checkbox-all :checkbox="$checkbox"
                    :theme="$theme->checkbox" />
            @endif
            @foreach ($columns as $column)
                <x-livewire-powergrid::cols :column="$column" :theme="$theme"
            ...
            @if (isset($actions) && count($actions))
                <th class="{{ $theme->table->thClass . ' ' . $column->headerClass }}"
                    scope="col" style="{{ $theme->table->thStyle }}"
    ...
    <x-slot name="rows">
        <x-livewire-powergrid::inline-filters :makeFilters="$makeFilters"
            :checkbox="$checkbox" :actions="$actions" :columns="$columns"
            :theme="$theme" :enabledFilters="$enabledFilters" />
            ...
            <th>
                <tr class="{{ $theme->table->trBodyClass }}"
                    ...
                    <td class="{{ $theme->table->tdBodyClass }}"
                        ...
                        colspan="{{                         colspan="($checkbox ? 1 : 0) + (isset($actions) ? 1 : 0) + count($columns) }}">
        ...
        @else
            @foreach ($data as $row)
                <tr class="{{ $theme->table->trBodyClass }}"
                    ...
                    <x-livewire-powergrid::row :currentTable="$currentTable"
                        :primaryKey="$primaryKey" :theme="$theme" :row="$row"
                        :columns="$columns" />

                    <x-livewire-powergrid::actions :theme="$theme" :row="$row"
                        :actions="$actions" />
                </tr>
            @endforeach
        @endif
