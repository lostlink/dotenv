...
<div>
    @if (config('livewire-powergrid.filter') === 'inline')
        <tr class="{{ $theme->table->trClass }}"
            class="{{ $theme->table->trStyle }}">

            @if (count($makeFilters))
                @if ($checkbox)
                    <td class="{{ $theme->table->tdBodyClass }}"
                        style="{{ $theme->table->tdBodyStyle }}"></td>
                @endif
                        ...
                        <td class="{{ $theme->table->tdBodyClass }}"
                            ...
                            @foreach (data_get($makeFilters, 'multi_select', []) as $index => $multiSelect)
                                @if (data_get($multiSelect, 'field') === $column->field)
                                    ...
                                    'inline' => true,
                                    'column' => $column,
                                    'options' => "[['id' => 'laravel , 'name' =>
                                    'Laravel'], ['id' => 'alpineJs', 'name' =>
                                    'Alpine JS'], ['id' => 'livewire', 'name' =>
                                    'Livewire']]"
                                    ])
                                @endif
                            @endforeach
