...
@if (filled($multiSelect))
    <div wire:ignore class="flex @if (!$inline) col-md-6 col-lg-3 @endif"
        ...
        @endif
        <select
            data-none-selected-text="{{ trans('livewire-powergrid::datatable.multi_select.select') }}"
    ...
    @push('power_grid_scripts')
        <script>
            $(function() {
                $('.select_picker_{{ data_get($multiSelect, 'data_field') }}')
                    .selectpicker();
            })
            ...
            $('.select_picker_{{ data_get($multiSelect, 'data_field') }}')
                .selectpicker();
            $('select.select_picker_{{ data_get($multiSelect, 'data_field') }}').on(
                'change',
                function() {
