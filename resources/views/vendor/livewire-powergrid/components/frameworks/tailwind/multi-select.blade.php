...
<div wire:ignore x-cloak>
    <div x-data="dropdown('{{ $column->field }}', '{{ $multiSelect['data_field'] }}')"
        x-init="loadOptions()" x-cloak>
        <input name="values" type="hidden" readonly
            x-bind:value="selectedValues()">
        <div class="inline-block relative w-full p-2"
            style="min-width: 180px !important;">
            <div class="flex flex-col items-center relative">
                <div x-on:click="open"
                    class="w-full multi_select_{{ $column->field }}"
                ...
                <div class="w-full px-4">
                    <div x-show="isOpen()" x-cloak x-transition.origin.top
                        ...
                        <div class="flex flex-col w-full">
                            <template x-for="(option,index) in options"
                                :key="index">
                                <div>
                                    <div class="cursor-pointer w-full border-gray-100 rounded-t border-b dark:hover:bg-gray-500 dark:hover:text-gray-300 dark:border-gray-400"
                                        @click="select(index,$event)">
                                        <div x-bind:class="option.selected ? 'border-teal-600' : ''"
                                            class="flex w-full items-center p-2 pl-2 border-transparent border-l-2 relative">
                                            <div
                                                class="w-full items-center flex">
                                                <div class="dark:text-gray-200 pl-0 align-middle cursor-pointer hover:text-black hover:text-current text-left text-xs font-medium text-gray-600 tracking-wider leading-6"
                                                    x-model="option"
    ...
    <script>
        function dropdown(field, data_field) {
            return {
                ...
                loadOptions() {
                    const options = document.getElementById('select_' + field)
                        .options;
                    for (let i = 0; i < options.length; i++) {
                        this.options.push({
                            ...
                            selected: options[i].getAttribute('selected') !=
                                null ? options[i].getAttribute('selected') :
                                false
                        });
                    }
                }
    ...
    <select class="hidden" x-cloak id="select_{!! $column->field !!}"
        wire:model.lazy="filters.multi_select.{!! $multiSelect['data_field'] !!}">
        <option value="">
            {{ trans('livewire-powergrid::datatable.multi_select.all') }}
        </option>
        @foreach ($multiSelect['data_source'] as $relation)
            <option value="{{ $relation['id'] }}">
                {{ $relation[$multiSelect['display_field']] }}</option>
        @endforeach
    </select>
