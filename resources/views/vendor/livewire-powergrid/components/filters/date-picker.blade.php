...
<div>
    ...
    @push('power_grid_scripts')
        <script type="application/javascript">
            flatpickr(document.getElementsByClassName(
                'range_input_{{ data_get($date, 'dataField') }}'), {
                mode: 'range',
                defaultHour: 0,
                @json(config('livewire-powergrid.plugins.flat_piker.locales.' .
                app()->getLocale())),
                ...
                onClose: function(selectedDates, dateStr, instance) {
                    if (selectedDates.length > 0) {
                        window.livewire.emit('eventChangeDatePiker', {
                            ...
                            field: instance._input.attributes[
                                'data-field'].value,
                            values: instance._input.attributes[
                                'data-key'].value,
